                                                  ...
                                                  ConstProperties
{
    ...
    // Slots are stored in a single hash map with a special key that supports the hash
    // table contract and is also comparable. Both are important because java.lang.HashMap
    // falls back to an ordered comparator if the map has too many collisions
    private LinkedHashMap<SlotKey, Slot> slotMap;
    ...
    private static abstract class SlotKey<T extends Object>
        implements Serializable, Comparable<SlotKey<?>>
    {
        ...
        public boolean equals(Object o) {
            try {
                return key == null ? o == null : key.equals(((SlotKey<?>)o).key);
    ...
    private static final class StringSlotKey
        extends SlotKey<String>
    {
        StringSlotKey(final String k) {
            super(k);
        }

        @Override
        public int compareTo(SlotKey<?> o) {
            // This kind of key should compare after integers and before symbols
            if (o instanceof IntegerSlotKey) {
                return 1;
            }
            if (o instanceof SymbolSlotKey) {
                return -1;
            }
            if (o instanceof StringSlotKey) {
                return key.compareTo(((StringSlotKey)o).key);
            }
            throw new AssertionError();
        }
    }

    private static final class IntegerSlotKey
        extends SlotKey<Integer>
    {
        IntegerSlotKey(final int i) {
            super(i);
        }
        ...
        public int compareTo(SlotKey<?> o) {
            ...
            if (o instanceof IntegerSlotKey) {
                return key.compareTo(((IntegerSlotKey)o).key);
            }
            throw new AssertionError();
        }
    }

    private static final class SymbolSlotKey
        extends SlotKey<Symbol>
    {
        SymbolSlotKey(final Symbol s) {
            super(s);
        }
        ...
        public int compareTo(SlotKey<?> o) {
            ...
            if (o instanceof SymbolSlotKey) {
                SymbolSlotKey sk = (SymbolSlotKey)o;
                if (key.hashCode() < sk.hashCode()) {
                    return -1;
                }
                if (key.hashCode() > sk.hashCode()) {
                    return 1;
    ...
    private static class Slot implements Serializable
    {
        ...
        SlotKey<?> name; // This can change due to caching
        private volatile short attributes;
        volatile Object value;

        Slot(SlotKey<?> name, int attributes)
        {
            this.name = name;
    ...
    private static final class GetterSlot extends Slot
    {
        ...
        GetterSlot(SlotKey<?> name, int attributes)
        {
    ...
    static SlotKey<?> makeSlotKey(String name, int index) {
        if (name != null && index != 0) {
            throw new IllegalArgumentException(name);
        }
        if (name == null) {
            return new IntegerSlotKey(index);
        }
        return new StringSlotKey(name);
    }
    ...
    {
        return null != getSlot(new StringSlotKey(name), SLOT_QUERY);
    }
    ...
    public boolean has(int index, Scriptable start)
    {
        ...
        return null != getSlot(new IntegerSlotKey(index), SLOT_QUERY);
    }
    ...
    {
        return null != getSlot(new SymbolSlotKey(key), SLOT_QUERY);
    }
    ...
    {
        Slot slot = getSlot(new StringSlotKey(name), SLOT_QUERY);
        if (slot == null) {
    ...
    public Object get(int index, Scriptable start)
    {
        ...
        Slot slot = getSlot(new IntegerSlotKey(index), SLOT_QUERY);
        if (slot == null) {
            return Scriptable.NOT_FOUND;
        }
        return slot.getValue(start);
    ...
    {
        Slot slot = getSlot(new SymbolSlotKey(key), SLOT_QUERY);
        if (slot == null) {
            return Scriptable.NOT_FOUND;
        }
        return slot.getValue(start);
    ...
    {
        if (putImpl(new StringSlotKey(name), start, value))
            return;
    ...
    public void put(int index, Scriptable start, Object value)
    {
        ...
        if (putImpl(new IntegerSlotKey(index), start, value))
            return;
    ...
    {
        if (putImpl(new SymbolSlotKey(key), start, value))
            return;
    ...
    {
        StringSlotKey k = new StringSlotKey(name);
        checkNotSealed(k);
        removeSlot(k);
    }
    ...
    {
        IntegerSlotKey k = new IntegerSlotKey(index);
        checkNotSealed(k);
        removeSlot(k);
    }
    ...
    {
        SymbolSlotKey k = new SymbolSlotKey(key);
        checkNotSealed(k);
        removeSlot(k);
    }
    ...
    {
        if (putConstImpl(new StringSlotKey(name), start, value, READONLY))
            return;
    ...
    {
        if (putConstImpl(new StringSlotKey(name), start, Undefined.instance, UNINITIALIZED_CONST))
            return;
    ...
    {
        return findAttributeSlot(new StringSlotKey(name), SLOT_QUERY).getAttributes();
    }
    ...
    {
        return findAttributeSlot(new IntegerSlotKey(index), SLOT_QUERY).getAttributes();
    }

    public int getAttributes(Symbol sym)
    {
        return findAttributeSlot(new SymbolSlotKey(sym), SLOT_QUERY).getAttributes();
    }
    ...
    public void setAttributes(String name, int attributes)
    {
        StringSlotKey k = new StringSlotKey(name);
        checkNotSealed(k);
        findAttributeSlot(k, SLOT_MODIFY).setAttributes(attributes);
    }
    ...
    {
        setGetterOrSetter(makeSlotKey(name, index), getterOrSetter, isSetter, false);
    }

    private void setGetterOrSetter(SlotKey<?> key, Callable getterOrSetter,
                                   boolean isSetter, boolean force)
    {
        ...
        if (isExtensible()) {
            gslot = (GetterSlot)getSlot(key, SLOT_MODIFY_GETTER_SETTER);
        } else {
            Slot slot = getSlot(key, SLOT_QUERY);
        ...
        if (!force) {
            ...
            if ((attributes & READONLY) != 0) {
                throw Context.reportRuntimeError1("msg.modify.readonly", key.toString());
            }
        }
        if (isSetter) {
    ...
    {
        Slot slot = getSlot(makeSlotKey(name, index), SLOT_QUERY);
        if (slot == null)
            return null;
        if (slot instanceof GetterSlot) {
    ...
    {
        SlotKey<?> key = makeSlotKey(name, index);
        checkNotSealed(key);
        GetterSlot gslot = (GetterSlot)getSlot(key,
    ...
    public void defineProperty(String propertyName, Object value,
                               int attributes)
    {
        checkNotSealed(new StringSlotKey(propertyName));
    ...
    public void defineProperty(Symbol key, Object value,
                               int attributes)
    {
        checkNotSealed(new SymbolSlotKey(key));
        put(key, this, value);
                               ...
                               Method getter, Method setter, int attributes)
    {
        ...
        GetterSlot gslot = (GetterSlot)getSlot(new StringSlotKey(propertyName),
                                               SLOT_MODIFY_GETTER_SETTER);
        gslot.setAttributes(attributes);
    ...
    private void checkNotSealed(SlotKey<?> key)
    {
        if (isSealed()) {
            throw Context.reportRuntimeError1("msg.modify.sealed", key.toString());
        }
    }
     ...
     */
    private boolean putImpl(SlotKey<?> key, Scriptable start, Object value)
    {
        ...
        if (this != start) {
            ...
            if (count < 0) checkNotSealed(key);
            slot = getSlot(key, SLOT_MODIFY);
        }
     ...
     * or this != start and a READONLY slot was found.
     */
    private boolean putConstImpl(SlotKey<?> key, Scriptable start,
                                 Object value, int constFlag)
    {
        ...
        if (this != start) {
            ...
            slot = getSlot(key, SLOT_MODIFY_CONST);
            int attr = slot.getAttributes();
            if ((attr & READONLY) == 0)
                throw Context.reportRuntimeError1("msg.var.redecl", key.toString());
    ...
    private Slot findAttributeSlot(SlotKey<?> key, int accessType)
    {
        Slot slot = getSlot(key, accessType);
        if (slot == null) {
            throw Context.reportRuntimeError1("msg.prop.not.found", key.toString());
        }
        return slot;
    }
    ...
    {
        Slot slot = getSlot(new SymbolSlotKey(key), accessType);
        if (slot == null) {
     ...
     */
    private Slot getSlot(SlotKey<?> key, int accessType)
    {
        Slot slot = slotMap == null ? null : slotMap.get(key);
        ...
        return createSlot(key, slot, accessType);
    }

    private Slot createSlot(SlotKey<?> key, Slot slot, int accessType) {
        if (slotMap == null) {
            slotMap = new LinkedHashMap<SlotKey, Slot>();
        }
        ...
        if (slot != null) {
            ...
            if (accessType == SLOT_MODIFY_GETTER_SETTER
                ...
                newSlot = new GetterSlot(key, slot.getAttributes());
            } else if (accessType == SLOT_CONVERT_ACCESSOR_TO_DATA
                    && (slot instanceof GetterSlot)) {
                newSlot = new Slot(key, slot.getAttributes());
            ...
            // Replace existing slot with new one
    ...
    private void removeSlot(SlotKey<?> key) {
        ...
        if (slot != null) {
            ...
            if ((slot.getAttributes() & PERMANENT) != 0) {
                Context cx = Context.getContext();
                if (cx.isStrictMode()) {
                    throw ScriptRuntime.typeError1("msg.delete.property.with.configurable.false",
                            key.toString());
                }
                return;
            }
    ...
    Object[] getIds(boolean getNonEnumerable, boolean getSymbols) {
        ...
        for (Slot slot : slotMap.values()) {
            if ((getNonEnumerable || (slot.getAttributes() & DONTENUM) == 0) &&
                    (getSymbols || !(slot.name instanceof SymbolSlotKey))) {
                if (c == externalLen) {
    ...
    */
    ...
    protected Slot getSlot(Context cx, Object id, int accessType) {
        if (id instanceof Symbol) {
            return getSlot(new SymbolSlotKey((Symbol)id), accessType);
        }
        String name = ScriptRuntime.toStringIdOrIndex(cx, id);
        if (name == null) {
            return getSlot(new IntegerSlotKey(ScriptRuntime.lastIndexResult(cx)), accessType);
        } else {
            return getSlot(new StringSlotKey(name), accessType);
        }
    }
